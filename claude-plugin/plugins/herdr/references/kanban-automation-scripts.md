...
Set `HERDR_KANBAN_DRY_RUN=0` to let the runner create workspaces, start Claude panes, update assignment metadata, and close disposable panes.
...
Spawned Claude panes run in YOLO mode. They default to interactive claude because the sweeper closes disposable panes by watching for Herdr-visible `idle` or `done` agent states. `claude -p` (headless) exits the process and can leave the pane in `unknown`, so use headless mode only when another cleanup path owns those residual panes.

```bash
claude --dangerously-skip-permissions "<role prompt>"
```

YOLO applies only to spawned Claude panes. The runner itself should still avoid irreversible repository or external actions unless a future script explicitly opts into them.
...
HERDR_KANBAN_CLAUDE_MODE=interactive
HERDR_KANBAN_WORKER_CLAUDE_MODE=interactive
HERDR_KANBAN_REVIEWER_CLAUDE_MODE=interactive
HERDR_KANBAN_HUMAN_AUDIT_CLAUDE_MODE=interactive
```
...
6. Runs Claude in that workspace's root pane with `herdr pane run`, using `HERDR_KANBAN_WORKER_CLAUDE_MODE`.

The dispatcher attaches before starting Claude so the sweeper can distinguish a freshly launched worker from an unowned disposable pane. The worker prompt starts from the already attached pane and moves the card to ongoing:
...
6. Runs Claude in that workspace's root pane in YOLO mode with the reviewer skill prompt, using `HERDR_KANBAN_REVIEWER_CLAUDE_MODE`.
...
Reviewer launch attaches before starting Claude so Herdr-native card tracking points at the active review pane from creation.
