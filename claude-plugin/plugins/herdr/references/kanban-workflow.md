...
They default to dry-run mode. Set `HERDR_KANBAN_DRY_RUN=0` to let them create Herdr workspaces, spawn Claude panes, write assignment metadata, and close disposable panes.
Dispatch, review, and human-audit launchers attach newly created panes before starting Claude so the sweeper can protect freshly launched work from early cleanup.

Spawned Claude panes use YOLO mode:
