...
Before editing anything, check for a repo isolation policy (`CLAUDE.md`, `AGENT.md`, or `AGENTS.md` at the checkout root). If one exists, you MUST obey it. When the policy mandates per-task worktrees:

- Do all edits, tests, validation, and commits inside a dedicated task worktree. Never edit, test, or commit in the shared checkout, even for a one-line change.
- If the launcher already placed you inside an isolated task worktree, keep using it. Do not create a nested worktree.
- Otherwise create the task worktree per the policy's "Start a task" steps (branch off the policy's base branch, use the policy's branch-name and worktree-path conventions), then work from it.

With no policy, fall back to a dedicated task branch in the current checkout.

## Execute

1. Read the card contract.
2. Establish the isolated task worktree/branch per the Isolation section above.
...
After moving the card to `reviewing` or `blocked`, stop working and leave the session idle. The dispatcher and sweeper own the next pane lifecycle step — the sweeper reclaims the pane once it reads as idle.
