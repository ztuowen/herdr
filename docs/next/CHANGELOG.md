...
- Added runtime invocation for plugin v2 `client_speech.transform_transcript` hooks, including capped synchronous action output and timeout handling for transcript transforms before terminal insertion.
...
- Direct new-workspace keybindings such as `cmd+n` now work from the empty landing page instead of leaving the client on a screen where the advertised action was ignored. (#366)
