...
`app snapshot` prints one aggregate JSON response with server version/protocol metadata plus the current workspaces, tabs, panes, agents, and kanban items. Use it when a script needs a single joined view instead of calling each list endpoint separately.
...
herdr plugin action invoke <action_id> [--plugin ID] [--payload <json|->]
```
...
so qualified ids remain unambiguous even when plugin ids contain dots. Pass
`--payload` to provide transient JSON input to the action; Herdr exposes it to
the command as `HERDR_PLUGIN_PAYLOAD_JSON` and does not persist it.

Storage:

```bash
herdr plugin storage get --plugin ID <key>
herdr plugin storage set --plugin ID <key> <json|->
herdr plugin storage list --plugin ID [--prefix PREFIX]
herdr plugin storage delete --plugin ID <key>
```

`plugin storage` reads and writes opaque JSON values scoped to an installed
plugin id. Pass `-` as the value to `storage set` to read JSON from stdin.
Herdr owns durable storage mechanics; the plugin owns value schemas, migrations,
and cleanup policy.

Resources:

```bash
herdr plugin resource get --plugin ID --resource ID <item_id>
herdr plugin resource put --plugin ID --resource ID <item_id> <json|->
herdr plugin resource list --plugin ID --resource ID
herdr plugin resource delete --plugin ID --resource ID <item_id>
```

`plugin resource` reads and writes discoverable plugin-owned objects declared in
a v2 plugin manifest. Herdr maps each resource item to plugin storage and
validates ids and storage limits; the plugin owns the JSON schema. During the
Kanban migration, new board plugins should use resource collections with
`kind = "application/vnd.herdr.kanban-card+json"` instead of adding new
Kanban-specific host commands.

Events:

```bash
herdr plugin event emit --plugin ID --event NAME <json|->
```

`plugin event emit` publishes a plugin-scoped opaque event to socket clients
subscribed with `events.subscribe` and `type = "plugin.event"`. Resource writes
also publish `resource.put` events, and deletes publish `resource.delete` only
when they remove an existing item.
