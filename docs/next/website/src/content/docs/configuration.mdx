...
`toggle_kanban` toggles the Kanban board view. It defaults to `ctrl+k`.

`speech_to_text` toggles the speech-to-text voice recording flow. It defaults to `ctrl+q`.

`audio_summary` plays an audio summary of the active tab if on an agent terminal. It defaults to `ctrl+w`.
...
Configure real-time client-side speech-to-text transcription powered by Gemini Live API:
...
- `gemini_api_key`: Your Google Gemini API key. Required for client-side speech-to-text.
- `model`: The Gemini model used for transcription. Defaults to `"gemini-3.1-flash-live-preview"`.
- `system_instruction`: An optional global post-processing prompt (unset by default). When defined, it is used for both AI-agent and standard terminal panes if their specific prompts are unset.
- `agent_system_instruction`: An optional post-processing prompt for AI-agent panes. If unset, falls back to `system_instruction`. If both are unset, the final runtime fallback is:
...
- `terminal_system_instruction`: An optional post-processing prompt for standard terminal/shell panes. If unset, falls back to `system_instruction`. If both are unset, the final runtime fallback is:
...
- `summary_system_instruction`: An optional system prompt for the audio summary mode (summarizing tab text to speak aloud). If unset, the final runtime fallback is:
  `"You are an AI assistant. Summarize the text present on the screen layout. Be concise and conversational, as this summary will be read aloud to the user."`
...
## Kitty graphics and LaTeX math rendering

Kitty graphics support is enabled by default for supported terminal environments (such as Ghostty, WezTerm, Kitty, Konsole, Foot, and Rio).

When a supported terminal is detected, LaTeX math formulas in Markdown documents are compiled and rendered as inline/block graphics. If terminal support is not detected or the terminal's pixel dimensions cannot be queried, LaTeX math rendering automatically falls back to raw LaTeX text.
