...
`copy_on_select = false` を設定すると、Herdr のマウスドラッグ選択とダブルクリックによる単語コピーが無効になります。デフォルトは `true` です。
