...
commands with transient JSON payloads. Transcript transform/insert hooks remain
client adapter extension points. Herdr does not add `speech.*` socket methods,
persist raw audio, or store speech provider credentials in plugin payloads.
