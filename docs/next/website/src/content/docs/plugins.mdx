...
`resources`, `notifications`, `client-speech`, and `markdown`. Herdr lists
capabilities in `plugin.list` so authors and tooling can see the plugin's host
dependency contract. V1 manifests keep working; Herdr infers their declared
actions, events, panes, and link handlers for reporting.
...
`client-speech` capability. Herdr invokes `start_dictation` after recording
starts and `stop_dictation` when recording stops, using ordinary plugin action
commands with transient JSON payloads.

Herdr invokes `transform_transcript` before writing the final transcript to the
terminal. The action receives `HERDR_PLUGIN_PAYLOAD_JSON` with `hook`,
`phase = "transform"`, `workspace_id`, `is_agent`, and `transcript`. The action
may print a JSON string, a JSON object with `transcript` or `text`, or raw text
on stdout. Empty stdout keeps the current transcript. Failed or timed-out
actions are logged and skipped so dictation still inserts the built-in
transcript. `insert_transcript` remains a declared extension point while the
insertion contract is finalized.

Herdr does not add `speech.*` socket methods, persist raw audio, or store speech
provider credentials in plugin payloads.

## Markdown

Plugin v2 can declare Markdown extension points as ordinary actions and panes.
These hooks are for transient document transforms, exports, math rendering, and
alternate terminal preview UIs; Herdr's built-in Markdown renderer and render
cache remain host-local.
...
The `markdown` table may reference action ids for `transform_document`,
`export_document`, and `render_math`, and a pane id for `preview_pane`. Herdr
validates that each reference exists and that the manifest declares the
`markdown` capability. These hooks use the existing plugin action and pane
surfaces; Markdown render cache entries are not plugin resources by default.
