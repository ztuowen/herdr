...
`kind = "application/vnd.herdr.kanban-card+json"`. The legacy Kanban API remains
the compatibility source of truth for now; the mirrored resource collection
lets board plugins prove they can read card state through `plugin.resource.*`
without adding new Kanban-specific host methods.
