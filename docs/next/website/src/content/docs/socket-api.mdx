...
Kanban event subscriptions include `kanban.added`, `kanban.updated`, and `kanban.deleted`. Each pushed event includes the changed `item`; clients can optionally pass `uuid` on the subscription to receive events for one card.
