...
- inspect one aggregate app snapshot for joined workspace, tab, pane, agent, and kanban state
...
| Plugins | `plugin.link`, `plugin.list`, `plugin.unlink`, `plugin.enable`, `plugin.disable`, `plugin.action.list`, `plugin.action.invoke`, `plugin.log.list`, `plugin.storage.get`, `plugin.storage.set`, `plugin.storage.delete`, `plugin.storage.list`, `plugin.resource.list`, `plugin.resource.get`, `plugin.resource.put`, `plugin.resource.delete`, `plugin.event.emit`, `plugin.pane.open`, `plugin.pane.focus`, `plugin.pane.close` |
...
`app.snapshot` returns one joined view of the current server and app state:

```json
{"id":"req_snapshot","method":"app.snapshot","params":{}}
```

The response is `type: "app_snapshot"` with `server`, `workspaces`, `tabs`,
`panes`, `agents`, and `kanban_items`. The `server` object includes Herdr's
version and client/server protocol number. Use this endpoint when a tool needs
a consistent top-level inventory; use narrower list/get methods when you only
need one resource type. `kanban_items` is kept as compatibility snapshot state
during the board migration; new board plugins should consume declared plugin
resources and `plugin.event` subscriptions instead of depending on new
Kanban-specific host fields.
...
Store opaque JSON state for an installed plugin:

```json
{"id":"req_plugin_storage_set","method":"plugin.storage.set","params":{"plugin_id":"example.tool","key":"cache/recent","value":["w1","w2"]}}
```

Read, list, or delete storage values:

```json
{"id":"req_plugin_storage_get","method":"plugin.storage.get","params":{"plugin_id":"example.tool","key":"cache/recent"}}
{"id":"req_plugin_storage_list","method":"plugin.storage.list","params":{"plugin_id":"example.tool","prefix":"cache/"}}
{"id":"req_plugin_storage_delete","method":"plugin.storage.delete","params":{"plugin_id":"example.tool","key":"cache/recent"}}
```

Plugin storage is scoped by installed plugin id and stored under the plugin
state directory. Herdr validates the plugin id and key and owns durable
read/write mechanics; the plugin owns the JSON value shape, migrations, and
cleanup policy. Use `HERDR_PLUGIN_CONFIG_DIR` and `HERDR_PLUGIN_STATE_DIR`
directly when a plugin needs custom files, databases, or large artifacts.

Use resource collections for discoverable plugin-owned objects:

```json
{"id":"req_plugin_resource_put","method":"plugin.resource.put","params":{"plugin_id":"example.board","resource_id":"cards","item_id":"card-1","value":{"uuid":"card-1","title":"Card","status":"todo"}}}
{"id":"req_plugin_resource_get","method":"plugin.resource.get","params":{"plugin_id":"example.board","resource_id":"cards","item_id":"card-1"}}
{"id":"req_plugin_resource_list","method":"plugin.resource.list","params":{"plugin_id":"example.board","resource_id":"cards"}}
{"id":"req_plugin_resource_delete","method":"plugin.resource.delete","params":{"plugin_id":"example.board","resource_id":"cards","item_id":"card-1"}}
```

Resources are declared in the plugin manifest and backed by plugin storage.
They are useful when another tool needs to discover or subscribe to objects
without knowing raw storage keys.

Built-in extension resources use the same methods. During the Kanban migration,
cards are available at plugin id `herdr.kanban`, resource id `cards`, with
`kind = "application/vnd.herdr.kanban-card+json"`. The legacy `kanban.*`
methods remain compatibility wrappers while resource clients can list, get,
put, delete, and subscribe to card changes through `plugin.resource.*` and
`plugin.event`.
...
Kanban event subscriptions include `kanban.added`, `kanban.updated`, and
`kanban.deleted`. Each pushed event includes the changed `item`; clients can
optionally pass `uuid` on the subscription to receive events for one card.
These events remain a compatibility surface while board plugins migrate to
generic plugin resources and plugin-scoped events.

Plugin event subscriptions use `plugin.event`. Plugins emit them with
`plugin.event.emit` or `herdr plugin event emit`; pushed envelopes use
`event: "plugin_event"` and include `plugin_id`, the plugin-owned `event` name,
and an opaque JSON `payload`. Subscribers can pass optional `plugin_id` and
`event` filters.
Plugin resource puts also publish `plugin.event` envelopes with
`event = "resource.put"`. Deletes that remove an existing item publish
`event = "resource.delete"`; no-op deletes do not publish a resource event.
Those payloads include `resource_id` and `item_id`; put events also include the
written `value`.
