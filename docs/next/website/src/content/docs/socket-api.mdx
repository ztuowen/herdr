...
`event` filters.
Plugin resource writes also publish `plugin.event` envelopes with
`event = "resource.put"` or `event = "resource.delete"`. Those payloads include
`resource_id` and `item_id`; put events also include the written `value`.
