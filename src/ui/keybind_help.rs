...
pub(super) fn keybind_help_groups(app: &AppState) -> Vec<HelpGroup> {
    ...
    if app.mode == crate::app::Mode::Kanban {
        groups.push((
            "kanban",
            vec![
                help_entry("esc", "exit kanban mode"),
                help_entry("h / j / k / l / arrows", "navigate cards"),
                help_entry("H / L / shift+arrows", "move card left/right"),
                help_entry("space / enter", "view card details"),
                help_entry("c / y", "copy card UUID"),
                help_entry("d / delete", "delete card"),
            ],
        ));
    }
    ...
    groups.push((
        ...
        vec![
            ...
            help_entry(keybind_label(&kb.toggle_kanban), "toggle kanban"),
            help_entry(
